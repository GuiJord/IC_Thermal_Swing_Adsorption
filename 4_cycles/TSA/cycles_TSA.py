...
P_H = 1e5           #Pa     Operating Pressure (constant in TSA - no pressure swing)
y_feed_CO2 = 0.15   #       CO2 composition in the feed

#TSA Regeneration (Heating/Cooling) parameters ==============================================
T_regen = 373.15    #K      Regeneration (hot purge / heating jacket) Temperature
y_purge_CO2 = 1e-6  #       CO2 composition in the purge gas (lean/inert purge)
v_purge = 0.5       #m/s    Purge gas velocity (heating and cooling steps)
...
T_regen_ad = T_regen/T0             #   Dimensionless Regeneration Temperature
...
#Operation Constants ==============================================
efficiency = 0.72       #   Purge blower/fan efficiency (kept for optional energy analysis, unused in the mass/energy balances below)
...
def ode_system(t, variable_vec):
    ...
    if operation_step == 'adsorption':
        ...
        T_ad_wall[-1]   = T_ad[-1]
        #Wall Temperature (ambient - no jacket heating during adsorption)
        Tw_ad[0]        = T_amb_ad
        Tw_ad[-1]       = T_amb_ad
    ...
    elif operation_step == 'heating':
        #Pressure
        P_ad_wall[0]    = P_ad[0] - v_purge/dPdz_aux/2
        P_ad_wall[-1]   = P_H/P0
        #Velocity
        v_ad_wall[0]    = v_purge
        v_ad_wall[-1]   = 2*dPdz_aux*(P_ad_wall[-1]-P_ad[-1])
        #Composition (lean purge gas, essentially free of CO2)
        y_CO2_wall[0]   = (y_CO2[0]+y_purge_CO2*v_purge*Pe*dZ_ad*0.5)/(1+v_purge*Pe*dZ_ad*0.5)
        y_CO2_wall[-1]  = y_CO2[-1]
        #Temperature (purge enters hot, at the regeneration temperature)
        rho_g           = P0*P_ad[0]/(R*T0*T_ad[0])
        Pe_h            = epsilon*v0*L/K_z*rho_g*Cp_g
        T_ad_wall[0]    = (T_ad[0]+v_purge*Pe_h*dZ_ad*0.5)/(1+v_purge*Pe_h*dZ_ad*0.5)
        T_ad_wall[-1]   = T_ad[-1]
        #Wall Temperature (jacket heated to the regeneration temperature)
        Tw_ad[0]        = T_regen_ad
        Tw_ad[-1]       = T_regen_ad

        #Spatial Derivatives
        dydz_CO2_backward[0]    = -v_ad_wall[0]*Pe*(y_purge_CO2-y_CO2_wall[0])
        dydz_CO2_forward[-1]    = 0
        
        dTdz_backward[0]        = -v_ad_wall[0]*Pe_h*(T_regen_ad-T_ad_wall[0])
        dTdz_forward[-1]        = 0

        dTwdz_backward[0]       = 0
        dTwdz_forward[-1]       = 0

    elif operation_step == 'cooling':
        #Pressure
        P_ad_wall[0]    = P_ad[0] - v_purge/dPdz_aux/2
        P_ad_wall[-1]   = P_H/P0
        #Velocity
        ...
        y_CO2_wall[0]   = (y_CO2[0]+y_purge_CO2*v_purge*Pe*dZ_ad*0.5)/(1+v_purge*Pe*dZ_ad*0.5)
        y_CO2_wall[-1]  = y_CO2[-1]
        #Temperature (purge enters at ambient/feed temperature)
        rho_g           = P0*P_ad[0]/(R*T0*T_ad[0])
        Pe_h            = epsilon*v0*L/K_z*rho_g*Cp_g
        T_ad_wall[0]    = (T_ad[0]+v_purge*Pe_h*dZ_ad*0.5)/(1+v_purge*Pe_h*dZ_ad*0.5)
        T_ad_wall[-1]   = T_ad[-1]
        #Wall Temperature (jacket back to ambient to cool the bed down)
        Tw_ad[0]        = T_amb_ad
        Tw_ad[-1]       = T_amb_ad
...
def result_calc(t,variable_vec):
    ...
    if operation_step == 'adsorption':
        ...
        P_ad_in         = P_ad[0,:] - v_purge/dPdz_aux/2
        P_ad_out        = P_H/P0
        v_ad_in         = v_purge
        v_ad_out        = 2*dPdz_aux*(P_ad_out-P_ad[-1,:])
        y_ad_CO2_in     = (y_CO2[0,:]+y_purge_CO2*v_ad_in*Pe*dZ_ad*0.5)/(1+v_ad_in*Pe*dZ_ad*0.5)
        y_ad_CO2_out    = y_CO2[-1,:]
        rho_g           = P0*P_ad[0,:]/(R*T0*T_ad[0,:])
        Pe_h            = epsilon*v0*L/K_z*rho_g*Cp_g
        T_ad_in         = (T_ad[0,:]+v_ad_in*Pe_h*dZ_ad*0.5)/(1+v_ad_in*Pe_h*dZ_ad*0.5)
        ...
        plt.plot(t, y_ad_CO2_out,color='firebrick')
        plt.ylabel('y$_{CO2,out}$ [-]')
        plt.xlabel('Time [-]')
        plt.xlim(left=0)
        plt.tight_layout()
        plt.savefig('yCO2_out_heating_time.png',dpi=200)
...
def heating(variable_vec):
    #Hot lean-purge step: desorbs CO2 by raising bed/jacket temperature to T_regen,
    #pressure held ~constant at P_H (no pressure swing).
    global operation_step
    operation_step = 'heating'

    time_interval = np.array([0,t_heat])*v0/L
    ...
    for i in range(N_time_steps):
        P_ad_wall = np.zeros(N+1)
        P_ad_wall = WENO(P_ad[:,i],P_ad_wall)
        P_ad_wall[0] = P_ad[0,i] - v_purge/dPdz_aux/2
        P_ad_wall[-1] = 1
        v_ad[i] = dPdz_aux*(P_ad_wall[-1]-P_ad_wall[-2])
...
def cooling(variable_vec):
    global operation_step
    operation_step = 'cooling'

    time_interval = np.array([0,t_cool])*v0/L
...
T_ad_global[:]  = 1
y_CO2_global[:] = 1e-6
concentration   = np.array([1e-6,1-1e-6])*P_H/(R*T0)
...
# P_ad_global[:]  = np.load('P_ad_cooling.npy')
# T_ad_global[:]  = np.load('T_ad_cooling.npy')
# y_CO2_global[:] = np.load('y_CO2_cooling.npy')
# x_CO2_global[:] = np.load('x_CO2_cooling.npy')
# x_N2_global[:]  = np.load('x_N2_cooling.npy')
# Tw_ad_global[:] = np.load('Tw_ad_cooling.npy')
# variable_vec_initial = np.concatenate([P_ad_global,T_ad_global,y_CO2_global,x_CO2_global,x_N2_global,Tw_ad_global])
#======================================== Initial Condition - End

t_ads  = 15   #Adsorption step time
t_heat = 30   #Heating step time
t_cool = 40   #Cooling step time

steps_name = ['adsorption','heating','cooling']
steps_time = [t_ads,t_heat,t_cool]
steps_func = [adsorption,heating,cooling]
...
steps_feed    = ['adsorption']
steps_product = ['heating']
...
def cycles(variable_vec):
    ...
    purity_list = np.array([])
    recovery_list = np.array([])
    while stop_criterium:
        ...
        purity_list = np.append(purity_list,purity)
        np.save('purity_list.npy',purity_list)

        recovery_list = np.append(recovery_list,recovery)
        np.save('recovery_list.npy',recovery_list)
